...
public void Button_IsRRewardAdReady()
{
    ...
    if (isRewardReady)
    {
        HMDebugger.LogDebug("Rewarded ad ready");

    }else
    {
        HMDebugger.LogDebug("Rewarded ad not ready");
    }
}
...
private void RewardShowFailCallback()
{
    ...
    HMDebugger.LogDebug("Failed to play the reward video");
}
...
<!--}-->
<!---->
<!--HachiMgr.Instance.ShowRewardedAd(RVPositionName.RV_RaisePrize.ToString(), (callback)=> { }, null, true,"ads not ready.");-->
<!--```-->
